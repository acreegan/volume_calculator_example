...
def main():
    ...
    # Select a data file
    Tk().withdraw()
    data_filename = askopenfilename(title="Select a data file", initialdir=".")
    Tk().destroy()
    ...
    volume = calculate_volume(flow, conf["dx"], conf["initial"], conf["filter_window_length"], conf["filter_order"])
